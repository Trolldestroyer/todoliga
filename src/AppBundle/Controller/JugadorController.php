...
class JugadorController extends Controller
{
    ...
    public function indexPlayerAction($slug)
    {
        ...
        return $this->render(':jugador:jugadores.html.twig',
            [
                'equipo'=> $equipo,
            ]
        );
    ...
    public function insertJugadorAction($id, Request $request)
    {
        ...
        if ($request->getMethod() == Request::METHOD_POST) {
            ...
            if ($form->isValid()) {
                ...
                $m->persist($c);
                $m->flush();
                return $this->redirectToRoute('app_jugador_jugadores', ['slug' => $id]);
            }
        }
    ...
    public function updateJugadorAction($id, Request $request)
    {
        ...
        if ($request->getMethod() == Request::METHOD_POST) {
            ...
            if ($form->isValid()) {
                $m->persist($jugador);
                $m->flush();
                return $this->redirectToRoute('app_jugador_jugadores', ['slug' => $equipoid]);
            }
        }
        return $this->render(':jugador:form.html.twig', [
            'form' => $form->createView(),
